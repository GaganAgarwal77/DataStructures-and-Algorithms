...
int DeleteMin(struct HeapNode **Heap, int *n, int *indexes)
{
    int a = Heap[0]->node;
...
void DecreaseKey(int i, int data, struct HeapNode **Heap, int *indexes, int *n)
{
    Heap[i]->priority = data;
    BottomUpHeapify(i, Heap, indexes);
}
void DijkstrasAlgorithm(struct AdjacencyListNode **AdjacencyList, int source, int n)
{
    int a = n;
    int u;
    ...
    DecreaseKey(source, 0, Heap, indexes, &n);
    for (int i = 0; i < n; i++)
    {
    ...
    while (n > 0)
    {
        ...
        while (temp != NULL)
        {
            ...
            if (Visited[temp->i] == false)
            {
                ...
                if (Heap[indexes[temp->i]]->priority > Heap[indexes[u]]->priority + temp->weight)
                {
                    printf("Yes\n");
                    DecreaseKey(indexes[temp->i], Heap[indexes[u]]->priority + temp->weight, Heap, indexes, &n);
                    for (int i = 0; i < n; i++)
                    {
                        printf("%d %d ", Heap[i]->node, Heap[i]->priority);
                    }
...
int main()
{
    ...
    DijkstrasAlgorithm(AdjacencyList, 0, n);
    return 0;
}
/*
5
